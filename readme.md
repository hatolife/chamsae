...
詳細は [spec_v0.1.0.md](./docs/spec_v0.1.0.md) / [spec_v0.2.0.md](./docs/spec_v0.2.0.md) / [spec_v0.3.0.md](./docs/spec_v0.3.0.md) / [spec_v0.4.0.md](./docs/spec_v0.4.0.md) / [spec_v0.5.0.md](./docs/spec_v0.5.0.md) を参照。
...
| マイルストーン | 内容 | 優先度 |
|---------------|------|--------|
| 6.1 | 矢印キー・Home/End/Delete対応 (コンポジション中の自動確定) | 高 |
| 6.2 | 候補ウィンドウの複数候補表示・選択 (ユーザー辞書の前方一致) | 高 |
| 6.3 | 設定ホットリロード (IME再起動不要) | 中 |
| 6.4 | DPI対応・マルチモニター (候補ウィンドウ・設定画面) | 中 |
| 6.5 | コンポジションプレビュー (入力中の字母を逐次表示) | 中 |
| 6.6 | ログ出力 (デバッグ・トラブルシューティング用) | 低 |

**目標**: 日常使用に耐える品質、候補選択による入力効率向上
...
|---------------|------|--------|
| 7.1 | 漢字 (한자) 変換 | 高 |
| 7.2 | 2ボル式キーボードレイアウト対応 (標準韓国語配列) | 高 |
| 7.3 | 入力履歴・頻度学習辞書 | 中 |
| 7.4 | 記号・特殊文字入力モード (ㄱ+漢字キー方式) | 中 |
| 7.5 | 辞書前方一致・予測変換 | 低 |
...
| マイルストーン | 内容 | 優先度 |
|---------------|------|--------|
| 8.1 | Windows統合テスト (CI上でのIME動作検証) | 高 |
| 8.2 | コード署名 (SmartScreen警告の回避) | 高 |
| 8.3 | アプリケーション別設定プロファイル | 中 |
| 8.4 | winget / Microsoft Store 配布 | 中 |
| 8.5 | ユーザーガイド (使い方ドキュメント) | 低 |
| 8.6 | パフォーマンス最適化 (変換エンジン・メモリ) | 低 |
...
└── src/
    ...
    │   ├── candidate_window.rs # 候補ウィンドウ
    │   ├── tray_icon.rs       # システムトレイアイコン
    │   └── icon.rs            # GDI動的アイコン生成
    ├── win32/
    │   ├── mod.rs         # Win32モジュール
    │   └── window.rs      # 基本ウィンドウ作成
